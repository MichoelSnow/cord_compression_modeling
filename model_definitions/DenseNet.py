...
def transition_block(x, reduction, name):
    ...
    x = keras.layers.Conv2D(int(keras.backend.int_shape(x)[3] * reduction), 1, use_bias=False, name=name + '_conv')(x)
...
def conv_block(x, growth_rate, name):
    ...
    x1 = keras.layers.Conv2D(4 * growth_rate, 1, use_bias=False, name=name + '_1_conv')(x1)
    ...
    x1 = keras.layers.Conv2D(growth_rate, 3, padding='same', use_bias=False, name=name + '_2_conv')(x1)
...
def DenseNet(blocks, input_shape=None, pooling=None, classes=3):
    ...
    if not pooling:
        ...
        if pooling == 'avg':
            ...
            raise ValueError('Pooling must be set to None, avg, or max.')
